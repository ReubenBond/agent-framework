...
public sealed class ConversationsApiClient
{
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>A list of items in the conversation, or an empty list if the conversation doesn't exist.</returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Uri requestUri = new($"/v1/conversations/{conversationId}/items?order={order}&limit={limit}", UriKind.Relative);

            HttpResponseMessage response = await this._httpClient.GetAsync(requestUri, cancellationToken).ConfigureAwait(false);

            if (response.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                // Conversation doesn't exist yet, return empty collection
                return [];
            }
