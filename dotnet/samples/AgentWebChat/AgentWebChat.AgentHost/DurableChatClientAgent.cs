...
public sealed class DurableChatClientAgent : DurableAgent
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        DurableAgentThread durableThread = this.GetOrCreateThread(options, thread);

        // Notify the thread of new incoming messages
        await NotifyThreadOfNewMessagesAsync(durableThread, messages, cancellationToken).ConfigureAwait(false);
        ...
        await NotifyThreadOfNewMessagesAsync(durableThread, response.Messages, cancellationToken).ConfigureAwait(false);
    }
}
