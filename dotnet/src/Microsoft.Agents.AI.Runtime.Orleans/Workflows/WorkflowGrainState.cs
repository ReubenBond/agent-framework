...
/// </summary>
[GenerateSerializer]
public sealed class WorkflowGrainState
{
    /// <summary>
    ...
    /// <summary>
    /// Indicates whether there is a pending update to the workflow index.
    /// Set to true before updating the index, cleared after successful update.
    /// Used to ensure eventual consistency if the index update fails.
    /// </summary>
    [Id(10)]
    public bool PendingIndexUpdate { get; set; }

    /// <summary>
    /// Gets the ETag as an opaque string (hex-encoded version number).
    /// </summary>
    public string GetETag() => this.Version.ToString("x16", System.Globalization.CultureInfo.InvariantCulture);
