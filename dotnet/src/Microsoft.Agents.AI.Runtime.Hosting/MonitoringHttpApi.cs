...
/// </summary>
public static class MonitoringHttpApi
{
    /// <summary>
    ...
    {
        var status = await monitoringService.GetSystemStatusAsync(ct).ConfigureAwait(false);
    ...
    private static async Task<IResult> GetWorkersAsync(
        ...
        CancellationToken ct)
    {
        var workers = await monitoringService.GetWorkersAsync(ct).ConfigureAwait(false);
    ...
    private static async Task<IResult> GetWorkerAsync(
        ...
        CancellationToken ct)
    {
        var worker = await monitoringService.GetWorkerAsync(workerId, ct).ConfigureAwait(false);
        if (worker is null)
        {
    ...
    private static async Task<IResult> DrainWorkerAsync(
        ...
        CancellationToken ct)
    {
        var logger = loggerFactory.CreateLogger("MonitoringHttpApi");
        var success = await monitoringService.DrainWorkerAsync(workerId, ct).ConfigureAwait(false);
        if (!success)
        {
            return Results.Problem(
    ...
    {
        var logger = loggerFactory.CreateLogger("MonitoringHttpApi");
        var success = await monitoringService.EnableWorkerAsync(workerId, ct).ConfigureAwait(false);
        if (!success)
        {
            return Results.Problem(
        ...
        CancellationToken ct)
    {
        ...
        var metrics = await monitoringService.GetWorkflowMetricsAsync(window, ct).ConfigureAwait(false);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            await foreach (var evt in monitoringService.StreamEventsAsync(ct).ConfigureAwait(false))
            {
                var eventType = evt switch
                {
                    WorkflowMonitoringEvent we => we.EventType,
                    WorkerMonitoringEvent we => we.EventType,
                    SystemMonitoringEvent se => se.EventType,
                    _ => "unknown"
                };
