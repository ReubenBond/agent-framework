...
public static class ConversationsHttpApi
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Items is { Length: > 0 })
        {
            foreach (ItemParam itemParam in request.Items)
            {
                ...
                await storage.AddItemAsync(created.Id, itemToAdd, cancellationToken).ConfigureAwait(false);
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            ItemResource itemToAdd = itemParam.ToItemResource();
            ItemResource created = await storage.AddItemAsync(conversationId, itemToAdd, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await storage.ListItemsAsync(conversationId, limit ?? 20, ParseOrder(order ?? "desc"), after, cancellationToken).ConfigureAwait(false);
        return Results.Ok(result);
    }
