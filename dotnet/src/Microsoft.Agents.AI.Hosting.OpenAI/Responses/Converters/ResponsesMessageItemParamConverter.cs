...
public sealed class ResponsesMessageItemParamConverter : JsonConverter<ResponsesMessageItemParam>
{
    ...
    public override ResponsesMessageItemParam? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        return role switch
        {
            "user" => JsonSerializer.Deserialize(root.GetRawText(), OpenAIJsonContext.Default.ResponsesUserMessageItemParam),
            "assistant" => JsonSerializer.Deserialize(root.GetRawText(), OpenAIJsonContext.Default.ResponsesAssistantMessageItemParam),
            "system" => JsonSerializer.Deserialize(root.GetRawText(), OpenAIJsonContext.Default.ResponsesSystemMessageItemParam),
            "developer" => JsonSerializer.Deserialize(root.GetRawText(), OpenAIJsonContext.Default.ResponsesDeveloperMessageItemParam),
    ...
    public override void Write(Utf8JsonWriter writer, ResponsesMessageItemParam value, JsonSerializerOptions options)
    {
        switch (value)
        {
            case ResponsesUserMessageItemParam user:
                JsonSerializer.Serialize(writer, user, OpenAIJsonContext.Default.ResponsesUserMessageItemParam);
                break;
            case ResponsesAssistantMessageItemParam assistant:
                JsonSerializer.Serialize(writer, assistant, OpenAIJsonContext.Default.ResponsesAssistantMessageItemParam);
                break;
            case ResponsesSystemMessageItemParam system:
                JsonSerializer.Serialize(writer, system, OpenAIJsonContext.Default.ResponsesSystemMessageItemParam);
                break;
            case ResponsesDeveloperMessageItemParam developer:
                JsonSerializer.Serialize(writer, developer, OpenAIJsonContext.Default.ResponsesDeveloperMessageItemParam);
                break;
            default:
