...
public sealed class AgentInvocationContextTests
{
    ...
    public void Constructor_WithoutJsonOptions_UsesDefaultOptions()
    {
        ...
        // Assert
        Assert.NotNull(context.JsonSerializerOptions);
        Assert.Same(OpenAIJsonUtilities.DefaultOptions, context.JsonSerializerOptions);
    }
    ...
    [Fact]
    public void IdGenerator_AllowsCallingGenerateMethods()
    {
        // Arrange
        var idGenerator = new IdGenerator("resp_test", "conv_test", randomSeed: 12345);
        var context = new AgentInvocationContext(idGenerator);

        // Act
        string messageId = context.IdGenerator.GenerateMessageId();
        string functionId = context.IdGenerator.GenerateFunctionCallId();
        string reasoningId = context.IdGenerator.GenerateReasoningId();

        // Assert
        Assert.StartsWith("msg_", messageId);
        Assert.StartsWith("func_", functionId);
